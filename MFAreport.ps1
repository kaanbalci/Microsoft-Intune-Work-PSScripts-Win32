...
.EXAMPLE
  ...
  USE THIS - .\MFAreport.ps1 | Export-CSV c:\Users\Kaan\Desktop\MFACheck.csv -NoTypeInformation
    ...
    try {
      ...
      If (($MsolUser.StrongAuthenticationRequirements) -or ($MsolUser.StrongAuthenticationMethods)) {
        Switch ($MFAMethod) {
            ...
            "TwoWayVoiceAlternateMobile" { $Method = "Alternate Phone call verification" }
            "TwoWayVoiceOffice" { $Method = "Office Phone call verification" }
      ...
      [PSCustomObject]@{
        ...
        MFAConfigured        = if ($MsolUser.StrongAuthenticationMethods) {$true} else {$false}
        MFAType           = $Method
        MFAState       = if ($MsolUser.StrongAuthenticationRequirements) {$MsolUser.StrongAuthenticationRequirements.state} else {"-"}
      }
    }
    catch {
      [PSCustomObject]@{
...
elseif ($adminsOnly) {
  foreach ($admin in $admins) {
    ...
    [PSCustomObject]@{
      ...
      UserPrincipalName = $MsolUser.UserPrincipalName
      isAdmin           = $true
      "MFA Configured"     = if ($MsolUser.StrongAuthenticationMethods) {$true} else {$false}
      "MFA Default Type"= $Method
      "SMS token"       = if ($MsolUser.StrongAuthenticationMethods.MethodType -contains "OneWaySMS") {$true} else {"-"}
      "Phone call verification" = if ($MsolUser.StrongAuthenticationMethods.MethodType -like "TwoWayVoice*") {$true} else {"-"}
      ...
      MFAState = if ($MsolUser.StrongAuthenticationRequirements) {$MsolUser.StrongAuthenticationRequirements.state} else {"Disabled"}
    }
  }
}
